...
public class GuildManager {
    ...
    public GuildManager() {
        ...
        this.guilds = Int2ObjectMaps.synchronize(new Int2ObjectOpenHashMap<Guild>());
    ...
    public void clearInactiveGuilds() {
        ...
        for (Int2ObjectMap.Entry<Guild> entry : this.guilds.int2ObjectEntrySet()) {
            if (entry.getValue().lastRequested < Emulator.getIntUnixTimestamp() - 300) {
                toRemove.add(entry.getValue().getId());
            }
        }

        for (Integer i : toRemove) {
            this.guilds.remove(i.intValue());
        }
    }
    ...
    public void dispose() {
        var guildIterator = this.guilds.int2ObjectEntrySet().iterator();

        while (guildIterator.hasNext()) {
            var entry = guildIterator.next();
            if (entry.getValue().needsUpdate)
                entry.getValue().run();
