...
import com.eu.arcturus.habbohotel.bots.VisitorBot;
...
import com.eu.arcturus.messages.outgoing.inventory.AddPetComposer;
...
public class RoomUnitManager {
    ...
    public boolean hasBotsAt(final int x, final int y) {
        ...
        synchronized (this.currentBots) {
            this.currentBots.values().forEach(new Consumer<Bot>() {
                @Override
                public void accept(Bot object) {
    ...
    public void habboEntered(Habbo habbo) {
        ...
        if (doorTileTopItem != null
                && !(doorTileTopItem instanceof com.eu.arcturus.habbohotel.items.interactions.InteractionTeleportTile)) {
            try {
