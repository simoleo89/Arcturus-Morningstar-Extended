...
import com.eu.arcturus.habbohotel.items.FurnitureType;
...
import com.eu.arcturus.habbohotel.wired.core.WiredMovementPhysics;
import com.eu.arcturus.messages.ISerialize;
...
import com.eu.arcturus.messages.outgoing.rooms.HideDoorbellComposer;
import com.eu.arcturus.messages.outgoing.rooms.UpdateStackHeightComposer;
import com.eu.arcturus.messages.outgoing.rooms.items.*;
import com.eu.arcturus.messages.outgoing.rooms.users.RoomUserStatusComposer;
import com.eu.arcturus.plugin.Event;
import com.eu.arcturus.plugin.events.furniture.FurniturePickedUpEvent;
import com.eu.arcturus.plugin.events.rooms.RoomLoadedEvent;
import com.eu.arcturus.plugin.events.rooms.RoomUnloadedEvent;
import com.eu.arcturus.plugin.events.rooms.RoomUnloadingEvent;
...
public class Room implements Comparable<Room>, ISerialize, Runnable {
  ...
  private static final HashMap<Integer, RoomMoodlightData> defaultMoodData = new HashMap<>();
  ...
  public final ConcurrentHashMap<RoomTile, HashSet<HabboItem>> tileCache = new ConcurrentHashMap<>();
  ...
  public Room(ResultSet set) throws SQLException {
    ...
    this.moodlightData = new HashMap<>(defaultMoodData);
  ...
  public void pickUpItem(HabboItem item, Habbo picker) {
    ...
    if (item.getBaseItem().getType() == FurnitureType.FLOOR) {
      ...
      HashSet<RoomTile> updatedTiles = new HashSet<>();
    ...
    synchronized (this.loadLock) {
      ...
      if (this.loaded) {
        ...
        try {
          ...
          com.eu.arcturus.habbohotel.wired.core.WiredManager.unregisterRoomTickables(this);

          // Clear wired engine caches for this room
          if (com.eu.arcturus.habbohotel.wired.core.WiredManager.getStackIndex() != null) {
            com.eu.arcturus.habbohotel.wired.core.WiredManager.getStackIndex().invalidateAll(this);
          }
          if (com.eu.arcturus.habbohotel.wired.core.WiredManager.getEngine() != null) {
            com.eu.arcturus.habbohotel.wired.core.WiredManager.getEngine().clearRoomRecursionDepth(this.id);
            com.eu.arcturus.habbohotel.wired.core.WiredManager.getEngine().clearRoomRateLimiters(this.id);
          }
  ...
  public Color getBackgroundTonerColor() {
    ...
    for (HabboItem object : this.itemManager.getRoomItems().values()) {
  ...
  public Map<Integer, Habbo> getHabboQueue() {
  ...
  public Map<Integer, Bot> getCurrentBots() {
    return this.unitManager.getCurrentBots();
  }

  public Map<Integer, Pet> getCurrentPets() {
  ...
  boolean canSitAt(HashSet<HabboItem> items) {
  ...
  boolean canLayAt(HashSet<HabboItem> items) {
  ...
  public void refreshGuild(Guild guild) {
    if (guild.getRoomId() == this.id) {
      HashSet<GuildMember> members = Emulator.getGameEnvironment().getGuildManager()
  ...
  public void refreshGuildColors(Guild guild) {
    if (guild.getRoomId() == this.id) {
      for (HabboItem habboItem : this.itemManager.getRoomItems().values()) {
